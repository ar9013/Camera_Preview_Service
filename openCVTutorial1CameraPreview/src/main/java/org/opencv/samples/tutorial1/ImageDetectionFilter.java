...
public final class ImageDetectionFilter  implements Serializable {
	...
	public ImageDetectionFilter(final Context context, final int referenceImageResourceID) throws IOException {
		...
		Mat mReferenceImage = Utils.loadResource(context, referenceImageResourceID, Imgcodecs.CV_LOAD_IMAGE_COLOR);
	...
	private void findSceneCorners() {
		...
		//Mat homography = Calib3d.findHomography(goodReferencePoints, goodScenePoints, Calib3d.LMEDS, reprojectionThreshold);
	...
	// 原本畫框線 為了 標示 找到圖片位置，做 debug 用 ，因為 正式版顯示 圖片資訊 所依註解掉
...
//		Imgproc.line(dst, new Point(mSceneCorners.get(0, 0)), new Point(mSceneCorners.get(1, 0)), mLineColor, 1);
//		Imgproc.line(dst, new Point(mSceneCorners.get(1, 0)), new Point(mSceneCorners.get(2, 0)), mLineColor, 1);
//		Imgproc.line(dst, new Point(mSceneCorners.get(2, 0)), new Point(mSceneCorners.get(3, 0)), mLineColor, 1);
//		Imgproc.line(dst, new Point(mSceneCorners.get(3, 0)), new Point(mSceneCorners.get(0, 0)), mLineColor, 1);
//
//	}
	...
	private void FilterSetting(){
		...
		try {
			...
			// 讀取 detector 參數
			mFeatureDetector.read(temp.getPath());
